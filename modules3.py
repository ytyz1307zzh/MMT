...
class CoAttention(nn.Module):
    def __init__(self, d_model, text_len, img_len):
        # d_model: feature vectors' dimension
        super(CoAttention, self).__init__()
        self.F = nn.Sequential(nn.Linear(d_model*d_model, 1),
                               nn.ReLU())
        self.MLP = nn.Linear(img_len*text_len, d_model)

    def forward(self, H_v, H_x):
        # H_v: batch x l x d, H_x: batch x m x d
        assert H_v.size()[0] == H_x.size()[0]
        batch_size = H_v.size(0)
        out = torch.stack([self.outer_product(H_v[i], H_x[i]) for i in range(batch_size)]) # (batch, d)
        return out

    def outer_product(self, V, X):
        assert len(X.size()) == 2
        text_len = X.size(0)
        img_len = V.size(0)
        dim = X.size(1)
        product = torch.einsum('ij,mn->imjn', X, V).view(text_len * img_len, dim * dim) # outer product + flatten, (m*l) x (d*d)
        product = self.F(product).squeeze()
        product = self.MLP(product)
        return product
