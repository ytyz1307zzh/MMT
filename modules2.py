...
class CoAttention(nn.Module):
    def __init__(self, d_model):
        ...
        #self.W_3 = nn.Linear(d_model, d_model)
        #self.W_4 = nn.Linear(d_model, d_model)
        self.softmax = nn.Softmax(dim=-1)
    ...
    def forward(self, H_v, H_x):
        ...
        H_x_hat = torch.bmm(A_x, H_x) # H_x_hat: batch x l x d
        H_v_hat = torch.bmm(A_v, H_v) # H_v_hat: batch x m x d

        H_v_hat = self.W_1(H_x) + self.W_2(H_v_hat)
        #H_x_hat = self.W_3(H_v) + self.W_4(H_x_hat)
